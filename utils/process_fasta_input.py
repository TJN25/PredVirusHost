...
def process_whole_file(file_path: str) -> dict[str, List[List[str]]]:
    d: Dict[str, List[List[str]]] = {}
    current_seq: str = ""
...
def process_genome(d: Dict[str, List[List[str]]], seq: str, line: tuple[int, str, str]) -> tuple[Dict[str, List[List[str]]], str]:
    if line[0] == 0:
        return d, seq
    if line[0] == 1:
        seq += f'{line[1]}\n'
    ...
    if line[0] == 2:
        if line[2] in d:
            d[line[2]][0].append(line[1])
            d[line[2]][1].append(seq)
            #current_genome['n'] += 1
        else:
            d[line[2]] = [[line[1]], []]
        seq = ""
