...
aa <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/sequences/protein/archaea_output", "/arVOG_res.txt", sep = ""), fill = T)
pp <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/sequences/protein/archaea_output", "/baPOG_res.txt", sep = ""), fill = T)
ee <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/sequences/protein/archaea_output", "/euVOG_res.txt", sep = ""), fill = T)
...
genome_lookup <- read.table("/Users/nicth99p/bin/projects/PredVirusHost/sequences/protein/archaea_fasta_output/fasta-headers.txt", sep = " ", header = F, comment.char = "", quote = "", fill = T)
genome_lookup <- genome_lookup%>%dplyr::rename(target.name = V1, genome = V2) %>% 
  filter(genome != "") %>% 
  mutate(across('target.name', \(x) str_replace(x, '>', '')))

archaealModels <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/predvirushost/data", "/archaeal_model_scores.txt", sep = ""), sep = "\t", comment.char = "", quote = "", fill = T, as.is = T, header = T)
phageModels <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/predvirushost/data", "/phage_model_scores.txt", sep = ""), sep = "\t", comment.char = "", quote = "", fill = T, as.is = T, header = T)
eukaryoticModels <- read.table(paste("/Users/nicth99p/bin/projects/PredVirusHost/predvirushost/data", "/eukaryotic_model_scores.txt", sep = ""), sep = "\t", comment.char = "", quote = "", fill = T, as.is = T, header = T)
